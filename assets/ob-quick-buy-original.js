const checkVariants = () => {
  const variantsWrapper = Array.from(document.querySelectorAll(".qb-product-variants"));
  let variantMissing = false;
  const buyButton = document.getElementById("qb-atc-button");
  variantsWrapper.forEach((wrapper) => {
    if(!variantMissing){
      
      const wrapperSelected = wrapper.querySelector("input:checked");
      if(!wrapperSelected){
         const wrapperName = wrapper.dataset.optionname;
          buyButton.innerHTML = "Please Select " + wrapperName;
...
const addQbPrintSelectors = async (prod) =>{ 
    const printSelectorDiv = document.getElementById("qb-pattern-selector");
    printSelectorDiv.classList.remove("hidden");
    if(printSelectorDiv){
      printSelectorDiv.innerHTML = "";
    }
  for(let i = 0; i<10; i++){
    printSelectorDiv.innerHTML += `<div class="h-12 w-12 rounded-full animate-mt"></div>`
  }
  ...
  const prodPrice = prod.featured_image? prod.price/100 : prod.price; //check to see iuf nosto or shoipfy product
  ...
  const prodStyle = tags.find((tag) =>{
    ...
    if(printSelectorDiv){
    printSelectorDiv.innerHTML = "";
    printSelectorDiv.innerHTML = html
    }
} 
...
const initialiseQbSlider = (totalSlides) =>{
    const container = document.getElementById('default-quick-buy-image');
  const outerWrapper = $ob("#qb-image-outer-wrapper");
  outerWrapper.addEventListener('dragstart', e => e.preventDefault());
  let thumbnails = Array.from(outerWrapper.querySelectorAll(".product-thumbnail"));
  if(totalSlides > 7){
    document.querySelector("#qb-image-outer-wrapper .product-thumbnails").style.justifyContent = "start";
  }else{
    
    document.querySelector("#qb-image-outer-wrapper .product-thumbnails").style.justifyContent = "center";
  }
  let screenWidth = 178;
...
window.buildQb = async (prod, isPrintSwap=false) =>{
  ...
  if(!isPrintSwap){
    ...
    if(tags.some((item)=>{
      ...
      printSelectorDiv.classList.add("hidden")
    }
  }
  ...
  qbImageWrapper.addEventListener('dragstart', e => e.preventDefault());
  qbImageWrapper.classList.remove("load-animate");
  qbImageWrapper.innerHTML = "";
  ...
  const qbHeader = document.getElementById("default-quick-buy-header-title");
  qbHeader.innerHTML = title.split("-")[0];
  qbHeader.classList.remove("load-animate")
  const qbSubHeader = document.getElementById("default-quick-buy-header-sub-title");
  qbSubHeader.innerHTML = title.split("-")[0];
  qbSubHeader.classList.remove("load-animate")
  const currencySymbol = document.body.getAttribute("currency-symbol")
  const qbPriceDiv = document.getElementById("default-quick-buy-prod-price");
  qbPriceDiv.innerHTML = currencySymbol + prodPrice.toFixed(2)
  const qbCompDiv = document.getElementById("default-quick-buy-comp-price");
  if(compPrice == prodPrice){
    qbCompDiv.style.display = "none";
  }else{
    qbCompDiv.style.display = "block";
  }
  qbCompDiv.innerHTML = currencySymbol + compPrice.toFixed(2)

  // Update size guide href with current product handle
  const sizeGuideLink = document.querySelector(".js-quick-buy-size-guide-show");
  if (sizeGuideLink && prodHandle) {
    sizeGuideLink.href = prodHandle;
  }
...
document.addEventListener("click", (e)=>{
  if(event.target.closest("#default-quick-buy-content .add-to-cart-button")){
  ...
  return allMatch;
    ...
    nameInput.value = chosenVariant.id;
    addQbToCart(prodForm, chosenVariant.id, currentProdInfo);
    e.preventDefault();
  }
