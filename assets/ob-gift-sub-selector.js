$(document).ready(function () {
  ...
  function getActiveSelections(){
      ...
      if(activeSelector == 'Pre-PaidSubscriptionType'){
         lengthCriteria = $('#redeem').find('.js-sub-length-tab.active-selection').data('tags')? $('#redeem').find('.js-sub-length-tab.active-selection').data('tags') : "";
        ...
        function addSubQuickBuy(array) {
          try{
              if (array.length === 1) {
                  ...
                  let url = "/products/" + productHandle + "?view=qb-gss";
                  fetch(url)
                  .then(response => {
                    if (!response.ok) {
        ...
        if (cardCriteria === "type-physical-gift-subscription") {
          console.log('stay')
            $('#gift-options-3')
                ...
                .attr('data-product-type', 'type-physical-gift-subscription');
            // $('#gift-options-3')
